...
public class Factura {
    ...
    public void generatePDF(ecx.unomas.factura.Factura factura, ServletOutputStream servletOutputStream) throws DocumentException, MalformedURLException, IOException {
        ...
        if (servletOutputStream != null) {
            ...
            String cred="";
            for (InfoAdicional infoAdicional : factura.getInfoAdicional()) {
                if (infoAdicional.getValor().equalsIgnoreCase("CREDITO")) {
                    cred=infoAdicional.getValor();
                    cred.concat("-");
                }
            }
            writer = PdfWriter.getInstance(document, new FileOutputStream(Config.AUTORIZADOS_DIR+cred+factura.getFechaEmision().replace("/", "-") + "-" + factura.getRazonSocialComprador().replace(" ","-") + "-" + factura.getClaveAcceso() + ".pdf"));    
            OperacionesForms.rutadocPDFgeneradook = Config.AUTORIZADOS_DIR +cred+ factura.getFechaEmision().replace("/", "-") + "-" + factura.getRazonSocialComprador().replace(" ","-") + "-" + factura.getClaveAcceso() + ".pdf";
        }
        ...
        if (!factura.getContribuyenteEspecial().isEmpty()) {
        ...
        // jimmy modifico  if (subTotal12>0){
        Deb.consola("ecx.unomas.pdf.Factura.generatePDF()mmmmmmmmmmmmm: subtotal 12" + subTotal12);
