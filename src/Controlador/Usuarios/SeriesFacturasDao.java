 ...
 * @author USUARIO
 */
public class SeriesFacturasDao extends Coneccion {

    ProgressBar msg = new ProgressBar(1000, "Mensaje Inicial");
    String tabla = "SeriesFacturas";
    private ArrayList<SeriesFacturas> lista = new ArrayList<SeriesFacturas>();

    public void guardar(SeriesFacturas tarea) {
        try {
            this.conectar();
...
//        return modelo;
//    }
    public void modificar(SeriesFacturas persona) {
        try {
            this.conectar();
    ...
    public ArrayList<SeriesFacturas> listarBuscarConCodigo(Integer cod) {
        ResultSet rs;
        try {
            this.conectar();
            PreparedStatement st;
            st = this.getCnx().prepareCall("Select * from " + tabla + " where codigo = " + cod + " order BY fac3 LIMIT 0, 50");
            rs = st.executeQuery();
            //this.lista= new ArrayList();
            while (rs.next()) {
                SeriesFacturas per = new SeriesFacturas();
    ...
    public ArrayList<SeriesFacturas> listar() {
        ResultSet rs;
        try {
            this.conectar();
            PreparedStatement st;
            st = this.getCnx().prepareCall("Select * from " + tabla + " order BY fac3 LIMIT 0, 50");
            rs = st.executeQuery();
            //this.lista= new ArrayList();
            while (rs.next()) {
                SeriesFacturas per = new SeriesFacturas();
    ...
    public SeriesFacturas getMaxNUumeroFactura(Integer codigEquipo) {
        ResultSet rs;
        SeriesFacturas serieFac = new SeriesFacturas();
        try {
            this.conectar();
            PreparedStatement st;
            st = this.getCnx().prepareCall("SELECT  * , MAX(fac3)  as maximo FROM seriesfacturas WHERE equipos_Codigo= " + codigEquipo);
            rs = st.executeQuery();
            //this.lista= new ArrayList();
            while (rs.next()) {
                SeriesFacturas per = new SeriesFacturas();
    ...
    public SeriesFacturas buscarConID(Integer id) {
        ResultSet rs;
        SeriesFacturas u = new SeriesFacturas();
        try {
            this.conectar();
            PreparedStatement st;
...
//    }
    ...
    public void eliminar(SeriesFacturas persona) throws Exception {
        try {
            this.conectar();
