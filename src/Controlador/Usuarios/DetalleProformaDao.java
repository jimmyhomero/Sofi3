 ...
 * @author USUARIO
 */
public class DetalleProformaDao extends Coneccion {

    private String tabla = "detalleproforma";
ProgressBar msg = new ProgressBar(1000, "Mensaje Inicial");
    private List<DetalleProforma> lista = new ArrayList<DetalleProforma>();
    ...
    public DefaultListModel Buscar_productos(String value) {
        ...
        try {
            ...
            JOptionPane.showMessageDialog(null, ex + "hhhhff");
        } finally {
            this.cerrar();
        }
...
//    }
//
//    public List<DetalleProforma> listar() {
//        ResultSet rs;
//        try {
//            this.conectar();
//            PreparedStatement st;
//            st = this.getCnx().prepareCall("select f.*, df.* from proformas f inner join detalleproforma df on f.Codigo = df.Proforma_Codigo where f.codigo = 3" );
//            rs = st.executeQuery();
//            //this.lista= new ArrayList();
//            while (rs.next()) {
...
//    }
    ...
    public DetalleProforma buscarConIDFact(Integer id) {
        ResultSet rs;
                DetalleProforma u = new DetalleProforma();
        try {
            this.conectar();
            PreparedStatement st;
...
//            select usuarios.*, tipos_usuarios.codigo,tipos_usuarios.tipo from usuarios inner join tipos_usuarios on tipos_usuarios.codigo=usuarios.Tipo_Usuario_codigo where usuarios.codigo = 1
            ...
            st = this.getCnx().prepareCall("select * from DetalleProforma where Proforma_codigo= "+ id);
            //select usuarios.*, tipos_usuarios.tipo from usuarios inner join tipos_usuarios on tipos_usuarios.codigo=usuarios.Tipo_Usuario_codigo

            rs = st.executeQuery();
            //this.lista= new ArrayList();
            while (rs.next()) {
                DetalleProforma per = new DetalleProforma();
                per.setCodigo(rs.getInt("Codigo"));
                ...
                per.setFactura_Codigo(rs.getInt("Proforma_Codigo"));
    ...
    public DetalleProforma buscarConColumayValorLike(String columna,String value) {
        ResultSet rs;
        DetalleProforma u = new DetalleProforma();
        try {
            this.conectar();
    ...
    public void eliminar(String proformaCodigo) throws Exception {
        try {             
            this.conectar();
            PreparedStatement st =this.con.prepareStatement("DELETE from DetalleProforma "
				+ "WHERE Proforma_Codigo=?");			            
            st.setString(1,proformaCodigo);
            st.executeUpdate();
        } catch (SQLException e) {
